...
(async () => {
    ...
    console.log(signal);
    draw("result1", signal);
    const { KissFFT, instantiateKissFFTModuleFromFile } = await import("../../dist/esm/index.js");
    const fftModuleUri = "../../libkissfft-wasm/libkissfft.js";
    const kissFFTModule = await instantiateKissFFTModuleFromFile(fftModuleUri);
    ...
    const fftw2Amp = (from) => {
        ...
        const phases = new Float32Array(length / 2);
        for (let i = 0; i < length / 2; i++) {
            const real = from[i * 2];
            const imag = from[i * 2 + 1];
...
(async () => {
    const injector = async () => {
        ...
        // @ts-ignore
        const { KissFFT, instantiateKissFFTModule } = kissfftwasm;
        const kissFFTModule = await instantiateKissFFTModule();
        const kissFFT = new KissFFT(kissFFTModule);
